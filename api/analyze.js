    ...
    players: {
      ...
      items: {
        ...
        properties: {
          ...
          comment: S("이 선수에 대한 코칭 코멘트. 정확히 2문장. 능력치는 등급 수준으로만 말하고 숫자로 환산하지 말 것. 단 '하이므로'처럼 등급 글자에 조사를 붙이지 말고 '느린 편이라'처럼 자연스러운 우리말로 쓸 것."),
...
- 등급 글자(상·중·하)에 조사를 바로 붙이지 마세요. 한 글자뿐이라 우리말로 읽히지 않습니다.
    ✗ "속도가 하이므로" / "체력이 상이라" / "피지컬 상과 롱패스로"
    ✓ "속도가 느린 편이라" / "체력이 좋으니" / "강한 몸싸움과 롱패스로"
  등급 글자를 꼭 쓰겠다면 명사로 끊으세요 — "속도 하 등급", "피지컬은 상".
...
- comment 문장에서도 마찬가지입니다. 등급보다 높거나 낮게 말하지 마세요.
  (등급 글자를 그대로 붙여 쓰라는 뜻이 아닙니다 — 위 [작성 규칙]의 표현 방식을 따르세요.)
...
${players
  .map(
    (p) =>
      ...
      (p.movedFrom
        ? `
    ※ 평소 ${p.movedFrom} 자리 선수입니다. 오늘은 ${p.pos}로 나섭니다${p.declaredSub ? " (본인이 겸할 수 있다고 한 자리)" : " (겸업 경험은 없다고 한 자리)"}.`
        : "") +
      `
...
- **※ 표시가 붙은 선수는 평소와 다른 자리에서 뜁니다.** 그 선수에게는 일반적인
  포지션 지시 대신, **원래 자리와 무엇이 달라지는지**를 먼저 말해주세요. 세 가지가
  들어가야 합니다 — ① 원래 자리에서 하던 것 중 오늘 하면 안 되는 것,
  ② 새 자리에서 새로 해야 하는 것, ③ 익숙하지 않아 실수가 나올 지점 하나.
  (예: CAM 이 CM 으로 내려온 경우 — "전방으로 먼저 뛰어나가던 습관을 오늘은 참으십시오.
   공을 받기 전에 뒤를 먼저 확인하고, 수비 뒤에서 받아 돌아서는 데 집중하세요.
   앞만 보고 전진 패스를 노리다 역습을 맞는 장면이 가장 위험합니다.")
  겸업 경험이 없다고 한 자리라면 지시를 더 단순하게, 하나만 확실히 시키세요.
